...
class SpecHistoryExporter:
    ...
    def extract_type_definition(content: str, type_name: str) -> str | None:
        ...
        logging.warning(f"Could not find type definition for {type_name} in schema")
        return None
    ...
    def save_type_definition(
        ...
        with open(file_path, "w") as f:
            f.write(type_def)
        logging.debug(f"Saved type definition for {parent_type} to {file_path}")
    ...
    def process_type_definitions(
        ...
        """
        logging.info(f"Processing type definitions for {len(new_concepts)} new and {len(updated_ids)} updated concepts")
        ...
        for concept_name in concepts_to_process:
            if concept_name not in concept_ids:
                logging.warning(f"No ID found for concept {concept_name}, skipping")
                continue
    ...
    def update_spec_history_model(
        ...
        if new_concepts:
            logging.info(f"Added {len(new_concepts)} new concepts:")
            for new_concept in new_concepts:
                logging.info(f"  {new_concept}")
        if updated_ids:
            logging.info(f"Updated IDs for {len(updated_ids)} concepts:")
            for updated_id in updated_ids:
                logging.info(f"  {updated_id}")
        ...
        if self.output:
            ...
            logging.info(f"Updated spec history saved to {self.output}")
        else:
