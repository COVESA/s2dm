...
export function identityColumnOf(table: LedgerTable | undefined): string {
	if (!table) {
		return "";
	}
	...
	return ownUri?.name ?? primaryKey?.name ?? table.columns[0]?.name ?? "";
}
...
export function identityColumnFor(
	...
	return identityColumnOf(tables.find((one) => one.name === table));
}
